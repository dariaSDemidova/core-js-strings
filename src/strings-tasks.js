...
function removeLastOccurrences(str, value) {
  const strIndex = str.lastIndexOf(value);
  if (strIndex === -1) return str;
  return str.slice(0, strIndex) + str.slice(strIndex + value.length);
}
...
function sumOfCodes(str) {
  ...
  return sepStr.reduce((sum, char) => sum + char.charCodeAt(0), 0);
}
...
function invertCase(str) {
  ...
  const invertedCharacters = characters.map((char) => {
